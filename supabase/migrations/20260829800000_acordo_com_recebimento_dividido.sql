-- Acordo com recebimento dividido: parte do honorário a empresa paga direto
-- ao escritório, o resto a cliente paga.
--
-- O caso real: o acordo de R$ 100.000 nunca passa inteiro pela nossa conta. A
-- empresa deposita uma parte do honorário direto para o escritório e manda o
-- restante para a conta da cliente. A cliente, com o dinheiro na mão dela,
-- paga o honorário que faltou.
--
-- O valor bruto continua sendo o acordo inteiro, porque é a estatística de
-- quanto se conseguiu para a cliente — mas o dinheiro dela nunca entra no
-- caixa e nunca vira repasse, porque cai direto na conta dela.
--
-- Na tela de baixa isso já existia (o "Pagamento dividido" do recebimento).
-- O que faltava era declarar a divisão lá no acordo, para o cronograma já
-- nascer com as duas trilhas em vez de ser corrigido parcela a parcela.

-- ============================================================
-- 1) Quanto do honorário vem direto da empresa
-- ============================================================
ALTER TABLE public.legal_receivables
  ADD COLUMN IF NOT EXISTS firm_direct_amount numeric(14,2) NOT NULL DEFAULT 0;

COMMENT ON COLUMN public.legal_receivables.firm_direct_amount IS
  'Parte do honorário que a empresa (parte contrária) deposita direto na conta do escritório. O restante do honorário é cobrado da cliente. Sai do mesmo valor esperado do escritório — não é dinheiro a mais.';

ALTER TABLE public.legal_receivables
  DROP CONSTRAINT IF EXISTS legal_receivables_firm_direct_check;

ALTER TABLE public.legal_receivables
  ADD CONSTRAINT legal_receivables_firm_direct_check
  CHECK (firm_direct_amount >= 0) NOT VALID;

-- ============================================================
-- 2) A parcela ganha uma terceira origem.
--    'principal'   — a cliente paga
--    'empresa'     — honorário contratual que a empresa paga direto
...
ALTER TABLE public.installments
  ...
  CHECK (stream IN ('principal', 'empresa', 'sucumbencia')) NOT VALID;

-- ============================================================
-- 3) Cadastro do acordo
-- ============================================================
CREATE OR REPLACE FUNCTION public.create_agreement_with_schedule(
  ...
  _firm_direct_amount numeric DEFAULT 0,
...
BEGIN
  ...
  IF COALESCE(_firm_direct_amount, 0) < 0 THEN
    RAISE EXCEPTION 'O valor pago direto pela empresa não pode ser negativo.';
  END IF;

  IF COALESCE(_firm_direct_amount, 0) > COALESCE(_expected_firm_amount, 0) + 0.01 THEN
    RAISE EXCEPTION 'A empresa não pode pagar direto (%) mais do que o escritório tem a receber (%).',
      _firm_direct_amount, _expected_firm_amount;
  END IF;
  ...
  INSERT INTO public.legal_receivables (
    ...
    fee_base_extra_amount, firm_direct_amount, success_fee_amount,
    cost_reimbursement, expected_firm_amount, expected_client_amount,
    agreement_date, flow, is_estimated, manual_override_reason
  ) VALUES (
    ...
    COALESCE(_fee_base_extra_amount, 0), COALESCE(_firm_direct_amount, 0),
    _success_fee_amount, _cost_reimbursement, _expected_firm_amount,
    _expected_client_amount, _agreement_date, _flow::public.flow_type,
    _is_estimated, NULLIF(btrim(_manual_override_reason), '')
  )
  ...
  IF _installments IS NOT NULL AND jsonb_array_length(_installments) > 0 THEN
    FOR _item IN SELECT * FROM jsonb_array_elements(_installments) LOOP
      INSERT INTO public.installments (
        ...
        CASE
          WHEN _item->>'stream' IN ('sucumbencia', 'empresa') THEN _item->>'stream'
          ELSE 'principal'
        END
      );
    END LOOP;
  ...
  INSERT INTO public.audit_logs (
    ...
    jsonb_build_object(
      ...
      'pago_direto_pela_empresa', COALESCE(_firm_direct_amount, 0),
...
CREATE OR REPLACE FUNCTION public.update_receivable(
  ...
  _firm_direct_amount numeric DEFAULT NULL
)
...
BEGIN
  ...
  IF _firm_direct_amount IS NOT NULL AND _firm_direct_amount < 0 THEN
    RAISE EXCEPTION 'O valor pago direto pela empresa não pode ser negativo.';
  END IF;

  -- A parte paga direto pela empresa sai de dentro do que o escritório tem a
  -- receber; nunca pode passar dele.
  _firm_final := COALESCE(_expected_firm_amount, _old.expected_firm_amount);
  _direto_final := COALESCE(_firm_direct_amount, _old.firm_direct_amount);

  IF _direto_final > _firm_final + 0.01 THEN
    RAISE EXCEPTION 'A empresa não pode pagar direto (%) mais do que o escritório tem a receber (%).',
      _direto_final, _firm_final;
  END IF;
  ...
  UPDATE public.legal_receivables SET
    ...
    firm_direct_amount = _direto_final,
  ...
  INSERT INTO public.audit_logs (
    ...
    jsonb_build_object(
      ...
      'pago_direto_pela_empresa', _old.firm_direct_amount
    ),
    jsonb_build_object(
      ...
      'pago_direto_pela_empresa', _direto_final
    )
  );
END;
$fn$;
...
-- ============================================================
-- 5) As assinaturas anteriores somem: com duas candidatas, o PostgREST não
--    sabe qual chamar e o cadastro para de funcionar.
-- ============================================================
...
REVOKE EXECUTE ON FUNCTION public.create_agreement_with_schedule(
  uuid, uuid, text, text, text, text, numeric, numeric, numeric, numeric,
  numeric, numeric, numeric, numeric, numeric, date, text, boolean, text, jsonb
) FROM PUBLIC, anon;
